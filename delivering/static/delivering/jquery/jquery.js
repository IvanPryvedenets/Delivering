$( document ).ready(function() {
            ...
            success: function( data ) { // If data has been transmitted
                ...
                $.each( data, function ( key, value ) { // Data sorting
                    ...
                    $( '.basket-info tbody' ).append('<tr>' + // Adding data to the basket page
                                                        ...
                                                        '<td class="table-input" valign="top"><input class="basket-input" type="number" min="1" max="10" value="' + value['number'] + '" data-id="' + value['unit id'] + '"></td>' +
    ...
    $( '.card-form' ).on( 'submit', function ( e ) {
        ...
        $( '.basket div' ).animate({'height': 25, 'width': 25, 'top': 24, 'padding-top': 3}, 400) // Change size of basket div

        setTimeout( function () {
            $( '.basket div' ).animate({'width': 19, 'height': 19, 'top': 27, 'padding-top': 0}, 300)
        ...
        if ( amount === 'None' ) {
            data['amount'] = null;
        } else if ( $.type( amount ) === 'string' ) {
            data['amount'] = parseFloat( amount.charAt( 2 ) * 0.1 ).toFixed( 1 ); // Change string to float
        }
